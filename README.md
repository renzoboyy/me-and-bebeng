# me and bebeng - Minecraft Server

Welcome to the **"jejed and bebeng! ♥♥"** Minecraft Server! This is a Fabric-based survival server for Minecraft 1.21.11, configured for two players (me and bebeng :D). It includes performance optimizations, crossplay support, and quality-of-life enhancements. <br>
<br>This server's IP configuration is networked via **Tailscale**, which is available for Windows and iOS, which jejed and bebeng use to play Minecraft with. Tailscale acts as a VPN from jejed's PC, which is hosting the server, to bebeng's iOS Device. <br><br>

Unfortunately, the world is not part of the commit as it is too big for a GitHub repository. To circumvent this, I uploaded a separate world backup download you can check out:<br>
[Direct Link](https://meandbebeng.fatdonut380.workers.dev) <br>
[Releases (for previous versions)](https://github.com/renzoboyy/me-and-bebeng/releases)

## Server Details
- **Minecraft Version:** 1.21.11
- **Platform:** Fabric
- **Gamemode:** Survival
- **Difficulty:** Hard
- **Max Players:** 2
- **Allocated RAM:** 2GB (`-Xmx2048M -Xms2048M`)

## Key Features & Installed Mods

### Performance Optimizations
- **Lithium** & **FerriteCore** & **ModernFix** & **ServerCore**: Heavily optimize server performance and memory usage.
- **C2ME** (Concurrent Chunk Management Engine) & **Noisium**: Drastically improve chunk generation speeds.
- **Chunky**: Allows for world pre-generation to minimize lag during exploration.

### Crossplay & Utilities
- **Geyser-Fabric**: Enables Bedrock Edition players to join this Java Edition server.
- **SkinsRestorer**: Restores player skins.
- **Better Fabric Console**: Enhances the server console output.

### Quality of Life & Gameplay Tweaks
- **Fabric Carpet**: A mod for vanilla tweaks and testing.
- **Trade Cycling**: Easily cycle through villager trades to get the desired outcomes.
- **No Crop Trample**: Prevents crops from being trampled when jumping on them.
- **Adventure Platform**, **Collective**: Essential libraries for other mods.
